...
from mobile_controls import MobileInterface # New Import
...
async def main():
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_SPACE:
                    waiting = False
        # Remove pg.time.wait(10) as it blocks the browser
        
    
    # No asyncio.sleep here for synchronous desktop version (Web uses loop above)
    ...
    if sys.platform == "emscripten":
        try:
             ...
             is_touch = window.eval('("ontouchstart" in window) || (navigator.maxTouchPoints > 0)')
             if is_touch:
                 mobile_ctrl = MobileInterface(surface.get_width(), surface.get_height())
                 # Mobile Mute Default
                 standard_use.SFX_MUTED = True
    ...
    timer_started = False 
    dt_factor = 1.0 # Initialize to avoid UnboundLocalError

    # Init Mobile Controls
    mobile_ctrl = None
    tips_menu = False # Initialize state variable
    ...
    if sys.platform == "emscripten":
        try:
             ...
             is_mobile = window.eval('("ontouchstart" in window) || (navigator.maxTouchPoints > 0)')
             if is_mobile:
                  mobile_ctrl = MobileInterface(surface.get_width(), surface.get_height())
        except:
             pass
    ...
    while running:
        ...
        dx = 0
        dx = 0
        # tips_menu = False # REMOVED: Do not reset state each frame!

        if main_menu:
             levels_page = 0 # Reset page when returning to main menu
             
             # Check for Tips menu state
             # We need a state variable for 'tips_menu' similar to others if we want it separate.
             # Or handle 'command == 30' loop here?
             # Let's add 'tips_menu' boolean to top scope or just reuse main_loop structure.
             pass

        if main_menu and not locals().get('tips_active', False): # Hacky? Let's use clean state variables.
             # Actually I need to define state variables at top of `main()`
             pass
        
        # REWRITE STATE LOGIC TO BE CLEANER
        if main_menu:
             levels_page = 0 
            ...
            for event in pg.event.get():
                ...
                # Mobile Events
                if mobile_ctrl:
                    mobile_ctrl.handle_event(event)
                    if mobile_ctrl.state['pause']:
                        pause_menu = True
                        # Reset state to prevent immediate re-trigger if logic changes
                        mobile_ctrl.state['pause'] = False
            ...
            mobile_keys = keys
            if mobile_ctrl:
                m_state = mobile_ctrl.get_state()
                # Create a wrapper or modify keys? Keys is a ScancodeWrapper (read-only usually).
                # We handle it by checking boolean flags alongside keys.
                
                # Override Input Handling
                # We can't write to `keys`.
                # We need to change how we Read keys.
                pass
            
            player.update_input_Player(keys) # This uses keys inside.
            # update_input_Player checks K_UP, K_w etc.
            # We might need to monkeypatch or pass extra args.
            # Simplest: Modify Player.update_input_Player to accept override dict?
            # OR Check mobile state here and trigger actions manually?
            
            # Jump
            if mobile_ctrl and mobile_ctrl.state['jump']:
                 player.request_jump()
            # Tongue
            if mobile_ctrl and mobile_ctrl.state['tongue']:
                 if not player.tongue_timer > 0: # Simple debounce/cooldown check
                     player.shoot_tongue()
            # Grapple
            if mobile_ctrl and mobile_ctrl.state['grapple']:
                # Auto-grapple nearest?
                # Similar to 'G' processing in event loop? 
                # Continuous holding 'grapple' might re-fire try_grapple?
                # try_grapple checks cooldown/state.
                target_tile = player.find_nearest_grapple_tile()
                if target_tile:
                     player.try_grapple(target_tile)
            
            # Horizontal input and facing
            input_left = keys[pg.K_LEFT] or keys[pg.K_a] or keys[pg.K_q]
            input_right = keys[pg.K_RIGHT] or keys[pg.K_d]
            
            if mobile_ctrl:
                if mobile_ctrl.state['left']: input_left = True
                if mobile_ctrl.state['right']: input_right = True
            ...
            if player.level_complete:
                ...
                if sys.platform != "emscripten":
                    ...
                    print("[Web] Skipping async task (Threading disabled)")
            ...
            # Delta time
            dt_ms = clock.tick(60)
            if dt_ms > 40: dt_ms = 40 # STRICT Cap (Max 2.4x speed) to prevent clipping
