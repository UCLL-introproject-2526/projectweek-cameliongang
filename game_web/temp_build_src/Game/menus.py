...
def draw_mainmenu(surface, font, network):
    ...
    if sys.platform != "emscripten" and exit_button.check_clicked():
        command = 'q'
...
def draw_login_menu(surface, font, network):
    ...
    if sys.platform == "emscripten":
        # WEB VERSION MESSAGE
        msg = font.render("For the best experience,", True, "gold")
        msg2 = font.render("download the desktop version!", True, "gold")
        surface.blit(msg, (panel_x + 100, panel_y + 150))
        surface.blit(msg2, (panel_x + 80, panel_y + 200))

        # Back Button Only
        btn_login_back.pos = (panel_x + (panel_w//2) - 90, panel_y + 350)
        btn_login_back.button.topleft = btn_login_back.pos
        btn_login_back.draw(surface, font)
...
def draw_leaderboard_menu(surface, font, network):
    ...
    if sys.platform == "emscripten":
         ...
         surface.blit(title, (surface.get_width()//2 - 100, panel_y + 40))
         
         msg = font.render("To submit scores,", True, "cyan")
         msg2 = font.render("play the desktop version!", True, "cyan")
         surface.blit(msg, (surface.get_width()//2 - 150, panel_y + 200))
         surface.blit(msg2, (surface.get_width()//2 - 180, panel_y + 250))
